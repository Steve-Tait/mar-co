...
const KeyPoint = ({ blok, className = "", isVertical = false, ...props }: { blok: KeyPointStoryblok; isVertical?: boolean; className?: string }) => {
	const { heading, body, button, icon = "home" } = blok;
	return (
		<motion.div className={cn("flex flex-col items-start gap-y-4 rounded-2xl bg-card p-4 text-left text-card-foreground sm:p-6", className)} {...props}>
