...
import { AnimatedWordsSectionStoryblok, AnimatedWordStoryblok } from "@/component-types-sb";
...
const SentenceLoop = ({ sentences = [], className, ...props }: { sentences: AnimatedWordStoryblok[]; className?: string }) => {
					...
					>
						{title.word.split(" ").map((el: string, i: number) => (
							<React.Fragment key={`${title._uid}-${i}`}>
								<motion.span className="inline-block" variants={textVariants} custom={i}>
									{el}
								</motion.span>
								{`${i + 1 === length ? "" : " "}`}
							</React.Fragment>
						))}
						<motion.span variants={textVariants} custom={length} key="dot" className="text-secondary">
							.
						</motion.span>
					</motion.p>
				);
