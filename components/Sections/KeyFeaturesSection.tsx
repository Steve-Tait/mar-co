...
const KeyFeaturesSection = ({ blok }: { blok: KeyFeaturesSectionStoryblok }) => {
	const { heading, body, buttons, features, isVertical } = blok || {};
	return (
		<Section blok={blok}>
			<Container className={cn("grid grid-cols-1 gap-8 xl:gap-16", isVertical ? "items-start md:grid-cols-3" : "")}>
				<div className={cn("col-span-1 flex flex-col gap-y-6", isVertical ? "md:sticky md:top-28" : "items-center text-center")}>
				...
				<motion.div
					className={cn(
						...
						isVertical
							? "col-span-2"
							: features?.length && !(features?.length % 2) && features?.length <= 4
								? "md:grid-cols-2"
