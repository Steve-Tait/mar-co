			...
			<Container className="max-w-5xl">
				<SectionWrap className="text-center" {...{ eyebrow, heading }}>
					<div className="grid grid-cols-2 gap-8 md:items-start lg:grid-cols-4">
						{phone && (
							<a className="group/contact col-span-1 flex flex-col items-center gap-y-4" href={`tel:${phone.replace(/\s/g, "")}`}>
						...
						{email?.email && (
							<a className="group/contact col-span-1 flex flex-col items-center gap-y-4" href={`mailto:${email.email}`}>
								...
								<span className="flex flex-col gap-y-1">{email.email}</span>
							</a>
						)}
						{linkedin?.url && (
							<a
							...
							>
								...
								<span className="flex flex-col gap-y-1">mar-co.digital</span>
							</a>
						)}
						{address && (
