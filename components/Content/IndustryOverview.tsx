...
const IndustryOverview = async ({ blok, searchParams }: IndustryOverviewStoryblok) => {
	const industries = await getIndustries();
	const { heading, description, image, body } = blok;
	return (
		<HeroWrap title={heading ?? "Industries"} excerpt={description} image={image}>
			<Section>
							...
							{industries.map((industry) => (
								<li key={industry.content.slug}>
									<Link
										...
										replace
										scroll={false}
										className={searchParams?.id && industry.uuid === searchParams.id ? "font-bold" : ""}
									>
