...
const ArticlesGrid = async ({ articles = [], categories = [], limit, toExclude = "", ...props }: TArticlesGrid) => {
	if (!articles?.length) {
		articles = await getArticles(categories, limit, toExclude);
	}

	if (!articles?.length) return <Heading className="text-center" level={6} heading="Sorry, no results found" />;
