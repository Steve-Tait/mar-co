...
export default async function ArticlesOverview({ blok, searchParams }: ArticleOverviewStoryblok) {
	const categories = await getCategories();
	const { title, excerpt, image, button, showSort, body } = blok;
	return (
		<HeroWrap {...{ title, excerpt, image, button }}>
			<Section blok={blok}>
				<Container className={showSort ? "grid grid-cols-1 gap-8" : "md:grid-cols-3 md:items-start lg:grid-cols-4"}>
					{showSort && (
								...
								{categories.map((category) => (
									<li key={category.content.slug}>
										<Link
											...
											replace
											scroll={false}
											className={searchParams?.id && category.uuid === searchParams.id ? "font-bold" : ""}
										>
					...
					<div className={cn(showSort ? "md:col-span-2 lg:col-span-3" : "col-span-1")}>
