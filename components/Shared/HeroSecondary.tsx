import { AssetStoryblok, CategoryStoryblok, IndustryStoryblok } from "@/component-types-sb";
...
const HeroSecondary = ({ title, excerpt, image, categories = [], industry = [], date, disclaimer, logo, className = "", ...props }: THero) => {
			...
			<div className="absolute bottom-0 right-0 top-0 aspect-square w-72 overflow-hidden rounded-bl-full bg-accent sm:left-1/4 sm:w-auto md:left-1/3 lg:left-1/2">
				{image?.filename && (
					<div className="relative size-full after:absolute after:inset-0 after:bg-black/50">
						<Image className="pointer-events-none block size-full object-cover" src={image.filename} alt={image.alt || image.title || ""} fill />
					</div>
				)}
			</div>
					...
					<div className="mb-4 flex items-end gap-x-4">
						...
						{tags && (
							<div className="flex flex-wrap gap-x-2">
								{tags.map((tag) => (
									<Badge href={`/${tag.full_slug}`} label={tag.name} key={tag.uuid} />
								))}
							</div>
						)}
					</div>
					{title && <Heading className="text-balance" heading={title} level={1} />}
					<div className="flex flex-col gap-y-2">
						{excerpt && <RichText className="max-w-prose text-balance" large content={excerpt} />}
						{disclaimer && <p className="text-xs text-primary-foreground/50">{disclaimer}</p>}
					</div>
				</div>
			</Container>
		</section>
