...
const ColouredContainer = ({
	...
	return (
		<motion.div
			...
			custom={isReverse}
		>
			<div className={cn("mx-auto aspect-square w-full max-w-40 sm:max-w-full", isReverse && "sm:order-1")}>
				<div className="relative size-full overflow-hidden">
					<motion.div className={cn(isReverse ? "h-[200%] w-full" : "flex h-full w-[200%]")} style={isReverse ? { y: translate } : { x: translate }}>
						{[...Array(2)].map((_, index) => (
							<span className={cn("block aspect-square w-full rounded-full", DOT_COLOR_MAP[color])} key={index} />
						))}
					</motion.div>
				</div>
			</div>
			<RichText className="prose prose-lg md:prose-xl sm:col-span-3" content={content} />
		</motion.div>
	);
};
