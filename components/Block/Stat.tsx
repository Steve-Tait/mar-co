...
export default function Stat({ stat, countNumbers = true, className }: { stat: StatStoryblok; countNumbers: boolean; className?: string }) {
	...
	return (
		<div className={cn("relative flex items-center gap-x-8 lg:gap-x-16", className)} ref={ref}>
			<motion.div className="1/2 size-12 shrink-0 rounded-full bg-secondary md:size-20" style={{ scale }} />
			<motion.div className="flex flex-col gap-y-2" style={{ opacity, y }}>
				<p className="text-2xl font-black sm:text-3xl">
					{prefix}
					{countNumbers ? <NumberCounter value={Number(value)} formatNumber={formatNumber} /> : value}
					{suffix}
				</p>
