...
const CaseStudies = async ({ caseStudies = [], industry = [], limit, toExclude = "", ...props }: TArticlesGrid) => {
	if (!caseStudies?.length) {
		caseStudies = await getCaseStudies(industry, limit, toExclude);
	}

	if (!caseStudies?.length) return <Heading className="text-center" level={6} heading="Sorry, no results found" />;
	...
	return (
		...
		>
			{caseStudies.map((caseStudy) => (
				<TileCaseStudy caseStudy={{ ...caseStudy.content, slug: caseStudy.slug }} key={caseStudy.uuid} />
			))}
		</div>
	);
};
