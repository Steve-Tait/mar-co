import { AssetStoryblok, ButtonStoryblok, CategoryStoryblok } from "@/component-types-sb";
...
const Hero = ({ title, excerpt, image, categories, button, className, ...props }: THero) => {
	return (
		<section
			className={cn(
				...
				{categories && (
					<div className="flex flex-wrap justify-center gap-x-2">
						{categories.map((category) => (
							<Badge href={`/${category.full_slug}`} label={category.name} key={category.uuid} />
						))}
					</div>
				)}
