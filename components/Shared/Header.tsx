...
import { HeaderStoryblok, MenuLinkStoryblok, MenuSectionStoryblok } from "@/component-types-sb";
	...
	return (
		...
		>
			<div className="flex items-center justify-between gap-x-5 lg:gap-x-8">
				<Link href="/" className="whitespace-nowrap font-heading text-2xl font-bold sm:text-4xl">
								...
								<div className="grid-cols-auto grid grid-flow-col gap-x-4 xl:gap-x-8">
									{menu.map((nestedBlok) => (
										<React.Fragment key={nestedBlok._uid}>
											{nestedBlok.items ? (
												<MenuSectionHeader blok={nestedBlok as MenuSectionStoryblok} />
											) : (
												<MenuLinkHeader blok={nestedBlok as MenuLinkStoryblok} />
											)}
										</React.Fragment>
									))}
								</div>
