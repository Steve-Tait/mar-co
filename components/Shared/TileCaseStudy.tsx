					...
					{industry?.length ? (
						<div className="pointer-events-none flex flex-wrap gap-2">
							{industry.map((industry) => (
								<Badge className="relative z-1" href={`/${industry.full_slug}`} label={industry.name} key={industry.uuid} />
							))}
						</div>
					) : null}
					{title && <h6 className="text-sm font-light">{title}</h6>}
				</div>
				<RichText className="grow text-balance font-bold uppercase" content={excerpt} />

				<MotionLink
					href={`/case-studies/${slug}/`}
					className="btn btn--text-link max-w-16 origin-right justify-end self-end overflow-x-clip whitespace-nowrap duration-300 before:absolute before:inset-0 group-hover/tile:max-w-40"
				>
					<span className="opacity-0 transition-opacity group-hover/tile:opacity-100">Read More</span>
