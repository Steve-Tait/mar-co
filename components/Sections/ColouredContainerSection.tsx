...
const COLOUR_MAP = ["muted", "primary", "secondary-muted", "secondary"] as const;
const ColouredContainerSection = ({ blok }: { blok: ColouredContainerSectionStoryblok }) => {
					...
					<div className="flex flex-col gap-y-6 lg:gap-y-10">
						{sections &&
							sections.map((section, index) => (
								<ColouredContainer blok={section} key={index} color={COLOUR_MAP[(index + 4) % 4]} isReverse={!!(index % 2)} />
							))}
					</div>
