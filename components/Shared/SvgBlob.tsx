...
export const SvgBlob = ({ color = "currentColor", image, shapeProps, ...props }: SvgBlobProps) => {
	...
	return (
		<svg {...svgProps} viewBox={`0 0 ${size} ${size}`} xmlns="http://www.w3.org/2000/svg">
			{image ? (
				<>
					<defs>
						<clipPath id={id}>
					...
					<image x="0" y="0" width="100%" height="100%" clipPath={`url(#${id})`} xlinkHref={image} preserveAspectRatio="none" />
				</>
			) : (
				<path d={svgPath} {...pathProps} />
			)}
