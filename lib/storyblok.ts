import { apiPlugin, storyblokInit, ISbStoriesParams, StoryblokClient } from "@storyblok/react/rsc";
import { resolveRelations } from "./consts";
import { Paths } from "./types";
import { ArticleStoryblok, CaseStudyStoryblok, CategoryStoryblok } from "@/component-types-sb";
...
export async function getLinks() {
	...
	const { data } = await storyblokApi.get("cdn/links", {
		version: (process.env.STORYBLOK_VERSION ?? "draft") as "draft" | "published",
	});
...
export const getArticles = async (categories: string[] = [], limit: number = 25, toExclude: string = ""): Promise<ArticleStoryblok[]> => {
...
export const getCaseStudies = async (industry: string[] = [], limit: number = 25, toExclude: string = ""): Promise<CaseStudyStoryblok[]> => {
	const storyblokApi = getStoryblokApi();
	let options: ISbStoriesParams = {
		version: (process.env.STORYBLOK_VERSION ?? "draft") as "draft" | "published",
