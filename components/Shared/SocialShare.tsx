...
const SocialShare: React.FC<IProps> = ({ className, format = "contextualNav" }) => {
	...
	const buttonClass = cn(
		...
		format === "post" && "w-full p-3 rounded-sm flex items-center justify-center gap-x-2.5",
	);
	const WINDOW: string = "toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=300, height=300";
	...
	return (
		<div className={className}>
			{format === "post" && <hr className="border-theme-border-secondary mb-6 lg:mb-12" />}

			<nav className={cn("flex origin-center items-center gap-0.5 overflow-hidden", format === "post" && "grid grid-cols-2 gap-2 md:grid-cols-4")}>
				{socials.map((social) => {
					return (
						<button key={social.name} aria-label={social.label} className={buttonClass} onClick={social.onClick}>
							{social.component}
							<span className={cn(format === "post" ? "body-text-small text-theme-text-link" : "sr-only")}>{social.labelOnPost}</span>
						</button>
					);
				})}
			</nav>
